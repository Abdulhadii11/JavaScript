...
orderBtn.onclick =()=>{
  ...
  async function patisseriePromise(){
    const value1= await checkOrder(order);
    const value2= await payment(value1);
    const value3= await stockControl(value2)
    console.log(value3);
  }
  patisseriePromise().catch((err)=>{
    console.log(err);
  })
}